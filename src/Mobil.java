...
public class Mobil {
    ...
    public static String getHeader() {
        return String.format(
                "| %-4s | %-10s | %-15s | %-12s | %-18s | %-12s |",
                "No", "ID Mobil", "Model", "Merk", "Harga Sewa", "Status");
    }

    public String getInfo() {
        DecimalFormat decimalFormat = new DecimalFormat("#,###.00");
        String hargaFormatted = "Rp " + decimalFormat.format(hargaSewa); // Format manual
        return String.format(
                "| %-10s | %-15s | %-12s | %-18s | %-12s |",
    ...
    public static ArrayList<Mobil> readFromCSV() {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader("daftarMobil.csv"))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                Mobil mobil = new Mobil(id, model, merk, hargaSewa, status);
